...
const Graph = ({user=null}) => {
  ...
  const testData = [
    { bgcolor: '#F18AAD', completed: (approved.length/totalApplications.length)*100, label: 'Aprovados' },
    { bgcolor: '#BDBAED', completed: (reproved.length/totalApplications.length)*100, label: 'Reprovados' },
    { bgcolor: '#E9FE47', completed: (pendingNotReproved.length/totalApplications.length)*100, label: 'Em processo de seleção' },
    { bgcolor: '#F1EEE6', completed: (paused.length/totalApplications.length)*100, label: 'Vagas pausadas' },
  ];
          ...
          <div className="h-full lg:px-7 px-5 py-0">
            <div className={twMerge('flex flex-col w-full lg:flex-row lg:gap-10 items-center')}>
              <div className={twMerge('lg:w-3/12 w-full ')}>
                <Doughnut data={data} updateMode='resize' options={{ maintainAspectRatio: false }} />
              </div>
