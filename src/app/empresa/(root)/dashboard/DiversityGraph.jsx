...
const DiversityGraph = ({user=null}) => {
  ...
  useEffect(()=>{
    if(applications){
      ...
      const blackDiversity = applications.filter(application => application.diversity.includes("black"));
      const lgbtDiversity = applications.filter(application => application.diversity.includes("lgbt"));
      const disabilityDiversity = applications.filter(application => application.diversity.includes("disability"));
      const elderlyDiversity = applications.filter(application => application.diversity.includes("elderly"));
 ...
 const options = {
    ...
    scales: {
      y: {
        ...
        ticks: {
          stepSize: applications? applications.length > 10? Math.floor(applications.length/10) : 1 : 0,
          color: 'white'
        },
    },
    x: {
  ...
  datasets: [
    ...
    {
      label: 'Aplicações',
      data: [blackApplicationsQtd, disabilityApplicationsQtd, lgbtApplicationsQtd, elderlyApplicationsQtd],
  ...
  return (
    <>
      ...
      <div className={twMerge('h-full w-full overflow-hidden lg:ml-0 lg:mb-6', style.background[theme])}>
