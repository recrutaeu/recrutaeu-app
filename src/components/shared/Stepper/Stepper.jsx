    ...
    <div className={twMerge('flex flex-col', className)}>
        ...
        <div className='flex items-center'>
            {steps.map((s, i) => { 
                ...
                return (
                    <>
                        <div className={twMerge('text-sm font-semibold border-2 w-8 h-8 rounded-full flex justify-center items-center drop-shadow-md', style.step[theme], stepDoneStyle)}>
                            {stepContent}
                        </div>
                        {i < (steps.length - 1)  ? <div className={twMerge('w-2 h-[2px]', style.stepCon[theme])} /> : null}
                    </>
                )
            })}
        </div>
